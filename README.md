...
- **Plan Trip** — Select a boarding and alighting stop on the route map for a journey-specific sun analysis
- **Interactive route map** — Road-snapped route geometry colour-coded by sun exposure (shade → mild → moderate → strong)
- **Favourites** — Star stops and services; synced to your account via Firestore
...
| Maps | Leaflet.js + CartoDB tiles + OSRM road geometry |
| Backend Proxy | Cloudflare Workers |
| Static Data Cache | Cloudflare R2 (refreshed daily by cron Worker) |
...
```
Browser (shiokbus.web.app)
    │
    ├── Static data (stops, routes, services)
    │       └──▶ Cloudflare Worker ──▶ Cloudflare R2 (daily cron sync from LTA)
    │
    ├── Real-time arrivals
    │       └──▶ Cloudflare Worker ──▶ LTA DataMall BusArrival v3 API
    │
    ├── Auth + Favourites
    │       └──▶ Firebase Auth + Firestore
    │
    └── Road geometry (route map)
            └──▶ OSRM public routing API
```

### Cloudflare Worker Security
- **CORS origin check** — only `shiokbus.web.app` and `shiokbus.firebaseapp.com` are allowed
- **App Check token required** — every request must carry a valid Firebase reCAPTCHA v3 token
...
Bus data is sourced from the **[LTA DataMall API](https://datamall.lta.gov.sg/)**. You will need a free API key from LTA to run your own instance.

---

## Acknowledgements

- [LTA DataMall](https://datamall.lta.gov.sg/) for Singapore bus data
- [Leaflet.js](https://leafletjs.com/) for maps
- [OSRM](https://project-osrm.org/) for road-snapped routing geometry
- [CartoDB](https://carto.com/) for map tiles
